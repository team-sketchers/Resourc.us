...
const App = () => {
  ...
  const [button, setButton] = useState(
    <Link to="/CreateResource" className="btn btn-success">
      Create Resource
    </Link>
  );
  ...
  useLayoutEffect(() => {
    if ((location === "/teams") | "/searchResults") {
      setButton(
        <Link to="/CreateTeam" className="btn btn-success">
          Create Team
        </Link>
      );
    } else if (
      location === "/CreateTeam" ||
      location === "/CreateResource" ||
      location === "/login" ||
      location === "/signup"
    ) {
      setButton("");
        ...
        <div className="innerContainer">
          <header className="mainHeader">
            <ul>
              ...
              {!isLogin() ? null : (
                <li>
                  <Link to="/" className="btn btn-secondary" onClick={logout}>
                    Logout
                  </Link>
                </li>
              )}
          ...
          {showResults && (
            <Redirect
              to={{ pathname: "/searchResults", state: { search: results } }}
            />
          )}
          <Switch>
